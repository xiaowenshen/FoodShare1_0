...
{
    public partial class SubmitInfo : CheckSession
    {
        ...
        protected void submitworks_Click(object sender, EventArgs e)
        {
                    ...
                    //   using(Image img=Image.FromFile(context.Request.MapPath(fulname)))
                    {
                        using (Bitmap map = new Bitmap(img.Width, img.Height))//根据上传成功的图片创建的img，宽度高度由此决定
                        {
                            ...
                            using (Graphics g = Graphics.FromImage(map))
                            {
                                ...
                                work.WTitle = Request["wtitle"].ToString();
                                work.path = fulname;
                                work.isdel = false;
                                work.UId = ((UserInfo)Session["uinfo"]).UId;
                                work.introduce = Request["wintroduce"].ToString();
                                ...
                                else
                                {
                                   ...
                                   Response.Redirect("SubmitInfo.aspx");
                                }
